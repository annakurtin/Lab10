...
wolf_track <- amt::make_track(wolf_summer3, x_coord1, y_coord1, datetime_new, crs = 4326) %>% amt::transform_coords(3857)
str(wolf_track)
...
stps <- amt::track_resample(wolf_track, rate = minutes(120), tolerance = minutes(1)) %>%
...
#the example here uses "wet" which I'm pretty sure is a binary of 1/0 if it's wet forest or not
conifer_raster <- raster(conif)
eda1 <- stps %>% amt::extract_covariates(conifer_raster, where ="end") %>% 
  mutate(conifer = if_else(conif == "TRUE", 1, 0)) %>% 
  mutate(landuse = factor(conif, levels = c(0, 1), labels = c("other", "conifer forest")))
# this area was throwing an error in my code in class

# Error in `mutate()`:
# ℹ In argument: `conifer = if_else(conif == "TRUE", 1, 0)`.
# Caused by error:
# ! [==] Can only match character values if x is categorical

# can do exploratory data analysis and plotting (maybe? might not need to do this)
```


```{r Generate Random Steps}
#Line 369
