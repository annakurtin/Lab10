...
```{r, warning = F}
# make the track and transform it to UTM
# this is a projected coordinate system that we can do euclidean math on 
dat_R <- amt::make_track(dat_1, x, y, t, crs = 4326) %>%
     amt::transform_coords(3857)
# error: ..1 must be a vector, not a <standardGeneric> object
...
*Kyle Bocinsky is a contributer to Fed Data 
...
# find the extent in UTM cartesian
ext(fisher_sf2)
# make a bounding box - will return the bounding of a simple feature or object
st_bbox(fisher_sf2)
# create an empty raster
mask.raster<-rast()
# define the extent of the bbox to the mask raster, making the boundaries a bit larger
ext(mask.raster) <- c(xmin=-8240000, xmax=-8160000, ymin=5260000 , ymax=5295000)

# define the resolution of the raster: NLCD is 30 m
res(mask.raster) = 30
...
# Check this - is fisher data bounded by the box?
```
...
str(values(land_use))
head(land_use)
# look at the values of land cover that occur in your area
...
# make the categories 
# filter out the empty classes 
rat <- as.data.frame(cats(land_use)) %>% 
  filter(!is.na(Class))
rat
levels(land_use)<-rat
# levels(land_use) <- rat[,-1]
land_use
#class(rat)
# set the active category of a raster
#activeCat(land_use,Class)
plot(land_use)
```
...
```{r, warning = F}
# reproject the fisher data to the same crs as the land_use
...
dat1 <- amt::make_track(dat_1, x, y, t, crs = 4326) %>%
  # transform the coordinates to be the same as wet
     amt::transform_coords(st_crs(wet))

# resample the steps of ricky t
...
# head(eda1)
# class(stps)
############# ERROR HERE: Error in extract_covariates.steps_xy(., wet, where = "start") : no raster ###################################
```
...
```{r}
# establsh to sample 9 available points
...
```{r}
# now let's look at the brownian cloud
...
```{r}
# new object: fit ISSF
## same structure as clogit
## this represents hypo that fisher selection varies w wet forest, step length, and selection varies based on time of day and that movement rate varies by time of day conditioned on unique step
## this is looking at the ends of the steps now
m3 <- d1 %>% amt::fit_issf(case_ ~ wet + sl_ + wet:tod_end_+ sl_:tod_end_ +strata(unique_step))
# this now has log of the step length times time of day
## if something affected step length, it would have a bigger effect than if something affected the log (log is a small number)
## This is just fitting whether you have a large or small effect on step length here 
m2 <- d1 %>% amt::fit_issf(case_ ~ wet + log_sl_ + wet:tod_end_+ log_sl_:tod_end_ + strata(unique_step))
# time of day interacts with both log of step length and step length: large effect
...
s
# they are 2.4 times more likely (under exp(coef) for wet) to select for this habitat relative to all others
# output wet:tod_end_night means that they are less likely to use wet forests during the night 
# interpreting step length hypothesis
# sl on itself is hard to interpret
# sl:tod_end_night (expressed in meters so very small) coef indicates that step lengths are longer at night (intercept is -0.004 and coef for intrxn with night is 0.005)
```
...
plot_sl(m1)
sl_m1<-sl_distr(m1)
# sl_distr obtains stp length/turning angle distributions from random steps or a fitted model 
## note we will extract the scale and shape parameters of the gamma distribution for later simulating the UD
# step length and log step length are including the scale and shape of a distrib
# with these kinds of models, it's only the interaction that makes sense
...
dat <- read_csv("Data/Martes pennanti LaPoint New York.csv") %>%
 ...
 filter(!is.na(`location-lat`)) %>%
  dplyr::select(x = `location-long`, y = `location-lat`,
              t = `timestamp`, id = `tag-local-identifier`) %>%
  # filter out specific animals
   filter(id %in% c(1465, 1466, 1072, 1078, 1016, 1469))

# nest them by ID so that when we pass it through AMT it goes by ID first
 dat_all <- dat %>% nest(-id)
dat_all$sex <- c("f", "f", "f", "m", "m", "m")
# nested dataframe with six columns 
dat_all
```
...
# we see here that we resample to the coarsest fix rate (10 min)
```
...
rat
# reclass the NLCD
# take the land cover codes and recode them to one two three and five to collapse them into the categories below
...
# 5: forest, herbaceous
 # 8: crops
# classify according to rcl
# classify is from terra, reclassifies based on argument rcl, right indicates that it will be (1-5] right = NA closes both sides so you just take the inside 
...
# build an SSF data frame all in one step 
   ...
   mutate(steps = map(trk, function(x) {
     x %>% amt::track_resample(rate = minutes(10), tolerance = seconds(120)) %>%
      amt::filter_min_n_burst() %>%
       ...
       amt::extract_covariates(lu, where = "both") %>% # this does both start and end 
       mutate(unique_step = paste(burst_,step_id_,sep="_")) %>% 
       mutate(landuse_end = factor(landuse_end))
     }))
# same error here: no raster ##########################################
```
...
m4 <- m1 %>% mutate(fit = map(steps, ~ amt::fit_issf(., case_ ~ landuse_end +
                                                           strata(unique_step))))
m4
# all of the coefficients in these outputs are relative to the water/wetlands category 
```
...
```{r}
# can break out the below code if we need to 
...
d2$x <- 1:nrow(d2)
d2
# two stage, marginal estimates
# this shows us the average coefficients across all individ
# we see that they are avoiding everything except land cover 3, which is forests
```
...
p1 <- m4 %>% mutate(coef = map(fit, ~ broom::tidy(.x$model, conf.int = T))) %>%
  ...
  geom_segment(mapping = aes(x = x - .4, xend = x + .4,y = mean, yend = mean), data = d2, inherit.aes = FALSE, linewidth = 1) +
...
p1
# this is NOT a mixed effects model, just handy ways to explore if you need a mixed effects model
# each box is marginal effects accross individual
# lines are each individual 
# from these plots, we see that everyone avoids developed areas and crops, while the "natural" areas are more variable
# we also don't see a consisitent difference between males and females
# this is a nice way to summaryze two stage sampling - taking the arithmatic average of these animals 
# if you have more animals, this plot will get super messy 
```
...
fisher6
# this looks like what we are starting with in our logistic regression stuff 
```
...
# this is a cross tabulation/legend field 
```
...
# this is just a logistic regression, nothing fancy 
# I is interaction expansion - takes a factor and tells GLM to treat it as a category 
...
# assumptions:
## IID - every row of data indep (we know to be false bc locations affected by previous ones)
## identiclly distrib (0:53:00 in afternoon lecture)
## again based on coefficients they avoid everything except for the "natural" areas 
```
...
fig5
# this is pretty similar to the SSF with mixed effects 
```
...
The difference in the interpretation from the different parameters highlights the 'effect' of movement, so to speak, on selection for covariates. Again, here, note we are not considering any differences between male or female, or night or day.  For example, one could conclude that the biggest difference is in the dplyr::selection of Developed Open, which we would underestimate the avoidance of if we failed to consider the movement processes. 
...
summary(clogit1)
coef(clogit1)
# notice here they aren't logistic regression coefficients, they're from a cox proportional hazards model
# notice that wet forests have disappeared
# we can say thaty they avoid land use cover 2,3, and 8 more than intercept, they select land use 5 more than the intercept 
...
*This shows that the conditioning isn't doing much. Since we're ignoring individuals, notice that the estimates have moved closer to the marginal effects from the SSF*
...
head
# add in random effects of ID
clogitM1<- coxme(Surv(time_,case_) ~ I(landuse_end) + strata(unique_step) + (1|id), data=fisher6)
AIC(clogitM1)
summary(clogitM1)
# Output:
## This shows us pretty much the same thing
# indicates that we don't need a bunch of random effects
## look at the variance it is 8.105e-05, which is vanishingly small. This means there aren't many differences between individuals 
...
# model 1 is max likelihood
# clogit is partial likelihood
```
...
## Fitting iSSF models with glmmTMB aka the right way to do it
*ISSF integrates movement and stap selection*
...
TMBm1$mapArg <-list(theta=factor(c(NA, 1:15))) # 15 random coefficients; four land cover types and six fishers
...
coefSum2
# 01:19:00
# underwhelmed by the differences between the models
# next thing you would want to look at is the standard error of the estimates
## the naive model will be unfairly precise in our estimates
# Assumptions: 1:24:00
# native everything indep
#clogit 
# coxme mvmt process and var bw individ is affecting resources selection
# two stage iSSF: overestimates, only valid for our three M and three F, allows animals to be constrained by mvmt and vary bw individuals
# glmmTMB: allows for indivi variation in resource selection and allows for movemnet process
# recall again that the coefficients are for males and females for two-stage iSSF, which is why there are 8 rows. 
# the model that you choose is affected by what you want to use it for (1:25:30)
# trying to make projections/predictions - glmmTMB - most correctly estimated uncertainty
p1

# link function for glmm.tmb.random is a poisson distribution 
# can only compare AIC of clogit models 
# can only compare AIC of models estimated with the same likelihood estimator 
